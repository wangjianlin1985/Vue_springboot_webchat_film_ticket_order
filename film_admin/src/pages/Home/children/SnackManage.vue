...
<script>
    import {getSnacks,updateSnackInfo,upLoadFile,deleteSnackInfo,getOptions} from '../../../api/index'
          ...
          async manageSnackInfo(){
            if (this.snackInfo.imageUrl&&this.snackInfo.firstTitle&&this.snackInfo.secondTitle&&this.snackInfo.price&&this.snackInfo.totalNumber) {
                if(this.$refs.uploadImg.files[0]){
                    ...
                    if (json.state===200){
                        if (json.data){
                            this.snackInfo.imageUrl = 'http://localhost:8081/weimai/upFile/'+json.data.img;
                        }
                    }else{
